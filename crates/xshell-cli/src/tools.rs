...
pub async fn execute(call: &ToolCall, root: &Path) -> String {
...
mod tests {
    ...
    fn blocks_paths_outside_root() {
        ...
        let result = resolve_existing(root, "/etc/hosts");
        assert!(result.is_err());
    }

    #[test]
    fn lists_current_project_directory() {
        let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    fn truncation_preserves_utf8_boundaries() {
        let input = "é".repeat(MAX_TOOL_OUTPUT_BYTES);
